...
from app.models import User, Post, Message, Notification, Comment, Like, UserDetails, RegisterAdmin  #  Added Comment Like UserDetails
...
import os   # Added
from flask_admin import Admin   # Added admin
from flask_admin.contrib.sqla import ModelView   # Added admin
...
def logout():
    logout_user()
    return redirect(url_for('login'))   # Changed from index
...
def user(username):
    ...
    prev_url = url_for('user', username=user.username, page=posts.prev_num) \
        if posts.has_prev else None
    form = EmptyForm()
    image_url=url_for('static', filename='profile_pics/'+current_user.image_file)    # Added
    return render_template('user.html', user=user, posts=posts.items, next_url=next_url, prev_url=prev_url, form=form, image_url=image_url)  # Addex , image_url=image_url
...
def edit_profile():
    ...
    if form.validate_on_submit():
        if form.picture.data:       # Added
            image_file=save_image(form.picture.data)        # Added
            current_user.image_file=image_file      # Added
        current_user.username = form.username.data
        current_user.about_me = form.about_me.data
        current_user.firstname = form.firstname.data    # Added
        current_user.lastname = form.lastname.data  # Added
        db.session.commit()
        ...
        form.firstname.data = current_user.firstname    # Added
        form.lastname.data = current_user.lastname  # Added
        image_url=url_for('static', filename='profile_pics/'+current_user.image_file)   # Added
    return render_template('edit_profile.html', title=_('Edit Profile'),
                           form=form, image_url=image_url)  # Addex , image_url=image_url
...
def search():
    ...
    user_id, total = User.search(g.search_form.q.data, page, app.config['POSTS_PER_PAGE'])  # Added
    user, total = User.search(g.search_form.q.data, page, app.config['POSTS_PER_PAGE'])  # Added
...
def account():
    ...
    if form.validate_on_submit():
        ...
        user_details=UserDetails(user_id=current_user.id)  #firstname=form.firstname.data,lastname=form.lastname.data, 
...
# admin = Admin(app, name='HolaCuenta.me - Admin', template_mode='bootstrap4')   # Added Admin
