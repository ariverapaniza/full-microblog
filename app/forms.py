...
from flask_wtf.file import FileField, FileAllowed    # Added
...
class RegistrationForm(FlaskForm):
    ...
    firstname = StringField(label='First Name', validators=[DataRequired(), Length(min=6,max=20)])  # Added  And deleted DataRequired()
    lastname = StringField(label='Last Name', validators=[DataRequired(), Length(min=6,max=20)])  # Added  And deleted DataRequired()
...
class EditProfileForm(FlaskForm):
    username = StringField(_l('Username'), validators=[DataRequired()])
    firstname = StringField(_l('First Name'), validators=[DataRequired(), Length(min=3, max=14)])  # Added
    lastname = StringField(_l('Last Name'), validators=[DataRequired(), Length(min=3, max=14)])    # Added
...
class AccountUpdateForm(FlaskForm):
    # firstname = StringField(label='First Name', validators=[Length(min=6,max=20)])
    # lastname = StringField(label='Last Name', validators=[Length(min=6,max=20)])
